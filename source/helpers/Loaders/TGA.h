...
internal loaded_bitmap LoadTGA(memory_arena *Arena, const char *FileName)
{
    loaded_bitmap Result = {};
    ...
    if (!File.Contents)
    {
        DebugLog("TGA: cannot open '%s'\n", FileName);
        return Result;
    }

    Result = ParseTGA(Arena, File.Contents, File.ContentsSize);
    DEBUGPlatformFreeFileMemory(File.Contents);

    if (Result.Pixels)
    {
        DebugLog("TGA '%s' loaded (%ux%u)\n", FileName, Result.Width, Result.Height);
    }
