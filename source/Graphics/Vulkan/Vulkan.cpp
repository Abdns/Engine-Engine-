...
internal void InitVulkan(HINSTANCE hinstance, HWND hwnd)
{
    ...
    if (!CreateCubePipeline(context))                  return;
    ...
    if (!CreateMeshes(context))                        return;
    if (!CreateTextures(context))                      return;

    DebugLog("Vulkan ready: 3D cube pipeline up\n");
}
...
internal void ShutdownVulkan()
{
    ...
    if (context->device != VK_NULL_HANDLE)
    {
        ...
        DestroyRenderPipeline(context, &context->Pipeline);
